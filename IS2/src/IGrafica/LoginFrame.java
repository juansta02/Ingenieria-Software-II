    ...
    public LoginView(AppController controller) {
        ...
        loginButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                boolean isAdmin = userText.getText().equalsIgnoreCase("admin");
                String ususario = userText.getText();
                String password = "";
                for (int i = 0; i < passwordText.getPassword().length; i++) {
                    password += passwordText.getPassword()[i];
                }
                

                if (User.login(usuario, password);) {
                    controller.showMainScreen(User.esAdmin);
                } else {
